...
class SqaState:
    ...
    def reward(self, gold, action_history = None):
        ...
        return float(len(setGold.intersection(setPred))) / len(setGold.union(setPred))
...
class SqaModel():
    ...
    def __init__(self, init_learning_rate, n_words):
        self.model = dt.Model()
        ...
        # similarity(v,o): (R^Tv)^T (R^To)
        self.R = self.model.add_parameters((SqaModel.WORD_EMBEDDING_DIM, SqaModel.WORD_EMBEDDING_DIM))
...
class SqaScoreExpressionState(SqaState):
    ...
    def __init__(self, nmodel, qinfo, vw, init_example = True):
        ...
        if init_example:
            ...
            self.ques_emb = dt.concatenate_cols([self.nm.E[self.vw.w2i.get(w, UNK)] for w in self.ques_word_sequence])
    ...
    def get_next_score_expressions(self, legit_actions):
        ...
        for act in legit_actions:
            ...
            #qwVecs = [dt.transpose(self.R) * qemb for qemb in self.ques_emb]
            qwVecs = dt.transpose(self.R) * self.ques_emb

            if act_type == SqaState.ActSelect:
                # question_embedding x column_name_embedding
                col = self.selectAct2Col(act)
                col_emb = self.headers_embs[col]
                colnameVec = dt.transpose(self.R) * col_emb
                # max_w sim(w,colname)
                #colScore = dt.emax([dt.dot_product(qwVec, colnameVec) for qwVec in qwVecs])
                colScore = dt.softmax(dt.transpose(qwVecs) * colnameVec)
                ...
                entryVec = dt.transpose(self.R) * self.entries_embs[r][c]
                # max_w sim(w,entry)
                #entScore = dt.emax([dt.dot_product(qwVec, entryVec) for qwVec in qwVecs])
                entScore = dt.softmax(dt.transpose(qwVecs) * entryVec)
                res_list.append(entScore)

            elif act_type == SqaState.ActWhereNul:
                #res_list.append(dt.dot_product(dt.average(qwVecs), self.NulW))
                res_list.append((dt.transpose(qwVecs) * self.NulW) / dt.scalarInput(float(len(self.ques_word_sequence))))
    ...
    def clone(self):
        ...
        res.ques_word_sequence = self.ques_word_sequence
...
def main():
    ...
    data_folder = "data"
    train_file="%s/random-split-1-train.first.tsv" % data_folder
    test_file="%s/random-split-1-dev.first.tsv" % data_folder
    ...
    neural_model = SqaModel(0.01, nwords)
    sm = BeamSearchInferencer(neural_model,1)
